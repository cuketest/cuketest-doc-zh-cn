...
- [演练：操作Qt应用中的ListView](#%e6%bc%94%e7%bb%83%e6%93%8d%e4%bd%9cqt%e5%ba%94%e7%94%a8%e4%b8%ad%e7%9a%84listview)
...
需要针对Qt的ListView组件开发的列表窗口进行操作和自动化测试。ListView通常用于含有大量可选项的窗口，比如文件列表、清单等等。

## 目标

为了完成对List组件的全面操作，这里定义了平时操作中List所有可能的操作，由浅入深的对List的自动化操作有个全面的认知。
- 完成对列表（List）的操作：滚动、滑动、检索内容；
- 完成对选项（ListItem）的操作：单击、双击，右键操作；
  
本次用于测试用的对象应用为Qt提供的Demo应用——FetchMore，其本质为一个简化的文件浏览工具，可以输入路径来检索路径下的文件/文件夹，如下：  
...
## 实际操作
由于桌面应用中，列表中的未显示的选项不会被直接识别到，也就是说，使用模型管理器对应用中的选项进行识别时，仅能识别到当前页中的选项，一旦滚动到其它页面，原来识别到的选项就会因为被隐藏而无法检测到。因此对于动态变化的选项作为识别对象是不理想的，这里选择了与[演练1：操作Qt应用中的表格]()中的操作类似，选择针对其固定的容器——也就是列表（List）作为识别对象，然后通过获取子控件的方式取得子控件。  

### 建立项目
#### 编辑剧本文件
新建项目后，首先将剧本文件完善，按照我们的目标将场景和步骤填上去，确认步骤描述准确以后就可以生成代码模版，步骤在后续的开发中可能会进行调整，但在这一步我们已经通过步骤描述对脚本编写有了语义化的认知和目标。完成剧本文件和代码模版以后如下：  
...
由于可见选项会随着滚动操作而动态变化，所以我们就没必要识别到具体的选项，而只要识别到其父控件也就是外部的List控件即可，如下红框所示的复选框可以不勾选：

![模型文件](./assets/list/fetchmore_model.png)

这样的模型文件就可以满足对列表和选项的简单操作了，但是在后续进一步的操作中偶尔也需要添加新的控件到模型文件中。  
...
由于Qt目前不支持使用`vScroll()`和`hScroll`进行垂直和水平滚动，但我们还有很多方法可以进行滚动：模拟按键（方向键和PageUp/Down键）进行滚动和翻页、使用滚动条按钮进行翻页、使用drag&drop进行拖拽/滑屏操作。  

> 这里使用滚动条的按钮滚动的方式考虑的场景——只有一条垂直的滚动条，比较简单。在一些时候，还会出现水平的滚动条，这个时候就要区分滚动条进行操作，但是由于Qt的组件不存在唯一标识符，所以在识别层会比较难区分，因此如果是这种情况，建议使用模拟按键的方式滚动。
...
这一个自动化中主要注意两个操作，一个是搜索框的输入，另一个是在搜索结果中检索是否有满足条件的项。

1. **搜索框的输入**：搜索框本质上是一个文本输入框，因此可以使用`set()`方法输入指定字符串。由于这个应用会自动的搜索，如果是需要另外输入`ENTER`回车键信号触发搜索的，可以像上一篇Workthrough中所写的，在后面追加`~`来模拟按下回车键。  

2. **检索结果**：前面提到过，由于滚动视窗的原理，只能获取到当前可见的选项，因此检索搜索结果，需要一边滚动一边判断当前页中是否有目标选项。这里有两个关键点：
  - 获取当前页所有的选项，并判断是否含有目标选项；
